...
import (
	...
	"ihome-client/service/user/utils"
)
...
func (e *User) UploadAvatar(ctx context.Context, req *user.UploadReq, resp *user.UploadResp) error {
	//存入到fastdfs中
	//change NewFdfsClient -> NewClientWithConfig
	...
	//把存储凭证写入数据库
	// change fdfsResp.RemoteFileId -> fdfsResp
	...
	// change fdfsResp.RemoteFileId -> fdfsResp
	uploadData.AvatarUrl = "http://192.168.137.81:8888/" + fdfsResp
